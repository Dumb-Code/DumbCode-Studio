...
export default function Dropup({ title, header, children, right = false, className = "", headerClassName = "" }:
    ...
    return (
        <div className={"text-right " + headerClassName}>
            <Menu as="div" className="relative inline-block text-left w-full">
                {({ open }) => (
                    <>
                        ...
                        >
                            <Menu.Items static className={(right ? "right-0" : "left-0") + " absolute bottom-8 mt-2 origin-top-right dark:bg-gray-900 bg-gray-200 dark:text-white text-black divide-y divide-black rounded shadow-lg focus:outline-none"}>
...
export const DropupItem = ({ name, onSelect, selected }: { name: string, onSelect: () => void, selected?: boolean }) => {
    return (
        <Menu.Item>
            {({ active }) => (
                <button className={`${selected ? "dark:bg-blue-700 dark:text-white bg-blue-400 text-black" : (active ? "dark:bg-gray-700 dark:text-white bg-gray-400 text-black" : "dark:text-white text-black dark:bg-gray-900 bg-gray-300")} group pr-5 flex rounded-md items-center w-full px-2 py-2 text-sm`} onClick={onSelect}>
                    {name}
                </button>
            )}
        </Menu.Item>
