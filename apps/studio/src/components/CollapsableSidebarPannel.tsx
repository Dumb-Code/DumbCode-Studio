...
const CollapsableSidebarPannel = ({ panelName, heightClassname, children, title }: PropsWithChildren<{ panelName: PanelName, heightClassname: string, title: string }>) => {
    ...
    return (
        <div className={(heightClassname === "h-full" && "h-full ") + "rounded-sm dark:bg-gray-800 bg-gray-200 flex flex-col pb-1 transition-height"}>
            ...
            <div className={(open ? heightClassname : "h-0 hidden") + " transition-height ease-in-out duration-200"}>
                {children}
            </div>
        </div>
    )
