...
const OptionButton = ({name, selected, setMotion}: {name: string, selected: boolean, setMotion: () => void}) => {
    return(
        <div>
            <button className={(!selected || "ring-2 ring-lightBlue-500") + " bg-gray-800 rounded w-80 text-white font-semibold p-2 text-left pl-4 my-1"} onClick={() => setMotion()}>{name}</button>
        </div>
    )
}
